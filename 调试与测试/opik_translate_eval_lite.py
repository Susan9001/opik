...
class QwenLiteJudgeModel(models.LiteLLMChatModel):
  ...
  def generate_provider_response(
    ...
    allowed_kwargs = {k: v for k, v in kwargs.items() if k not in disallowed_keys}
    return super().generate_provider_response(messages=messages, **allowed_kwargs)
...
class OpikTranslationEvaluator:
  ...
  def __init__(
      ...
      judge_model_name: str = "dashscope/qwen-plus",
  ) -> None:
    # 1. 数据集
